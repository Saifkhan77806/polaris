...
const Tab = ({
  ...
  const { activeTabId, previewFileId, setActiveTab, openFile, closeTab } =
    useEditor(projectId);

  const isActive = activeTabId === fileId;
  const isPreview = previewFileId === fileId;
  ...
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-2 h-8.75 pl-2 pr-1 cursor-pointer text-muted-foreground group border-y border-x border-transparent hover:bg-accent/30",
        isActive &&
